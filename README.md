...
버그 리포트에 **실제로 적힌** 검증 포인트를 추출해서(브레인스토밍이 아니라 텍스트 추출), 같은 버그 번호로 이미 작성된 TC가 그걸 빠짐없이 반영했는지 ☑/☐로 대조합니다. 

누락된 포인트만 골라 하나씩 생성할 수 있고, 방금 만든 항목은 **"◀ 이전 항목 / 다음 항목 ▶"** 버튼으로 재분석 없이 다시 확인/수정할 수 있습니다.
